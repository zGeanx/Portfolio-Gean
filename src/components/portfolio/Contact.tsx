    ...
    <section id="contato" className="border-b border-border">
      <div className="mx-auto max-w-[1160px] px-4 py-16 sm:px-6">
        <div className="border border-border bg-surface p-6 sm:p-10">
          ...
          <h2 className="mt-3 text-[clamp(1.6rem,5vw,2.5rem)] leading-tight font-semibold tracking-tight">
            {contact.title}
          </h2>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {items.map((item) => {
              ...
              return unset ? (
                <div
                  key={item.label}
                  className="flex items-center gap-3 border border-border bg-background p-4 opacity-70"
                >
                  {inner}
                </div>
              ) : (
                <a
                  ...
                  className="flex items-center gap-3 border border-border bg-background p-4 transition-colors duration-200 hover:border-primary"
                >
                  {inner}
                </a>
              );
